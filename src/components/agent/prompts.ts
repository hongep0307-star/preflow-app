...
\`\`\`scene
{ "scene_number": 1, "title": "", "description": "", "camera_angle": "", "location": "", "mood": "", "duration_sec": 8, "tagged_assets": [], "is_highlight": false, "highlight_kind": null, "highlight_reason": null }
\`\`\`
...
- 프로젝트에 등록된 에셋 라이브러리가 하나라도 존재하면, 모든 Shot은 **기본적으로 등록된 에셋을 최우선 활용**한다.
...
export const buildAssetUsageReminder = (assets: Asset[], lang: "ko" | "en" = "ko"): string => {
  ...
  if (lang === "en") {
    return [
      ...
      ...sections,
      "1) Use registered assets as the default choice when drafting or revising shots.",
      ...
      "4) Every shot's tagged_assets array MUST include ALL registered tags that appear in that shot.",
  ...
  return [
    ...
    "4) 각 Shot의 tagged_assets 배열에는 그 Shot에서 등장한 등록 태그를 전부 포함한다.",
